		...
		<div class="order_line">
			<table border="1" style="border-collapse: collapse; width: 100%">
				...
				<tr class="order-item-row">
					<td>{subject[name]}</td>
					<td>{class[name]}</td>
					<td><input class="order-item-quantity" name="fee[{i}][quantity]" value="{? echo $musters[$i] ?}" size="3" readonly /></td>
					<td><input class="order-item-fee" name="fee[{i}][fee]" value="{? echo $class['amount'] ?}" onChange="calculateOrder()" size="7" /></td>
					<td class="order-item-amount">{? echo product_price($musters[$i]*$class['amount']) ?}</td>
					<td><input class="order-item-discount" name="fee[{i}][discount]" value="{? echo $discounts[$i] ?}" size="7" /></td>
					<td><input class="order-item-discount-reason" name="fee[{i}][discount_reason]" value="{? echo html_escape(@$discount_reasons[$i]) ?}" size="20" /></td>
					<td class="order-item-total-amount">{? echo product_price($amounts[$i]) ?}</td>
				</tr>
				<?php } ?>
				<?php } ?>
			</table>
