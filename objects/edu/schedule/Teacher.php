<?php
class PzkEduScheduleTeacher extends PzkObject {
	public $layout = 'edu/schedule/teacher';
	public function getSchedules() {
		return _db()->select('schedule.*, classes.name as className, 
			...
			->where('classes.teacherId=\''.$this->getTeacherId().'\' and concat(year(studyDate),\'-\', week(studyDate, 1))=\'' .$this->getCurrentWeek() . '\'')->orderBy('studyDate asc, studyTime asc, room.name asc, classes.name asc')->result();
	}
}