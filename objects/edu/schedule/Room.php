<?php
class PzkEduScheduleRoom extends PzkObject {
	public $layout = 'edu/schedule/room';
	public function getSchedules() {
		return _db()->select('schedule.*, classes.name as className, 
			...
			->where('classes.roomId=\''.$this->getRoomId().'\' and concat(year(studyDate),\'-\', week(studyDate, 1))=\'' .$this->getWeek() . '\'')->orderBy('studyDate asc, studyTime asc, room.name asc, classes.name asc')->result();
	}
}