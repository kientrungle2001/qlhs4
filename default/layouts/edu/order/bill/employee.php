<form method="post" action="{url /employee/create_bill_post}">
<div class="order_wrapper">
	...
	<input type="hidden" name="employeeId" value="" />
		...
		<div class="order_line">
			<span class="order_line_label">Địa chỉ: </span>
			<span class="order_line_value"><input type="text" name="address"  style="width: 300px" /></span>
		</div>
		<div class="order_line">
	...
	function() {
		jQuery('.order_wrapper [name=order_name]').pzkAutoComplete({
			loader: function(word, callback) {
				pzks.dtable.employee.search(word, function(resp){
			...
			onSelect: function(item) {
				$('.order_wrapper [name=employeeId]').val(item.id);
